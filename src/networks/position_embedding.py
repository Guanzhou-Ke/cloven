...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    def __init__(self, embedding_dim, padding_idx=0, left_pad=0, init_size=128, batch_first=True):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        positions = make_positions(inputs, self.padding_idx, self.left_pad)
        if self.batch_first:
            return self.weights[device].index_select(0, positions.reshape(-1)).view(bsz, seq_len, -1).detach()
        else:
            return self.weights[device].index_select(0, positions.reshape(-1)).view(seq_len, bsz, -1).detach()
...
if __name__ == '__main__':
    pass
    # sp = SinusoidalPositionalEmbedding(128, batch_first=True)
    # x = torch.rand(10, 2, 128)
    # pos = sp(x[:, :, 0])
    # print(pos.shape)
    
    # sp = SinusoidalPositionalEmbedding(128, batch_first=False)
