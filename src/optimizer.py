...
def get_lr_scheduler(sc_name, opt_ins):
    
    if sc_name == 'cosine':
        sc = torch.optim.lr_scheduler.CosineAnnealingLR(opt_ins, T_max=5, eta_min=1e-6)
    elif sc_name == 'step':
        sc = torch.optim.lr_scheduler.StepLR(opt_ins, step_size=30, gamma=0.1)
    else:
        raise ValueError('lr_scheduler must be cosine or stepLR.')
    return sc
